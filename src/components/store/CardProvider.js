 ...
 const [productObj, setProductObj] = useState({});
  ...
  useEffect(() => {
    const asyncFun = async () => {
      let response = await fetch(
        `https://crudcrud.com/api/6d806138f1c54bc2b6f832dabc8059bb/cart${cartContext.email}`
      );
  ...
  const addItemToCartHandler = async (item) => {
    ...
    if (existingItem === -1) {
      ...
      let response = await fetch(
        `https://crudcrud.com/api/6d806138f1c54bc2b6f832dabc8059bb/cart${cartContext.email}/${allItems[existingItem]._id}`,
  ...
  const removeItemFromCartHandler = async (item) => {
    ...
    if (item.quantity <= 1) {
      const response = await fetch(
        `https://crudcrud.com/api/6d806138f1c54bc2b6f832dabc8059bb/cart${cartContext.email}/${item._id}`,
        { method: "DELETE" }
      );
